...
const SalesList = () => {
  ...
  const fetchSalesData = () => {
    start();
    sendGetRequest(`${SALES_OVERVIEW}?startDate=${filter?.selectedRange.start}&endDate=${filter?.selectedRange.end}&product=${filter?.product?.name ? filter?.product?.name : ""}&customer=${filter?.customer?.id ? filter.customer.id : ""}&per_page=${rowsPerPage}&page=${page}`, user.token)
      .then((_res) => {
        if (_res.status === 200) {
  ...
  const getValues = (filter) => {
    ...
    if (filter && filter.product && !filter.customer) {
      reportBy = " Sales Report by Product : " + filter.product.name
    } else if (filter && filter.customer && !filter.product) {
      reportBy = " Sales Report by Customer : " + filter.customer.name
    } else if (filter && (filter.product && filter.customer)) {
      reportBy = " Sales Report by Product : " + filter.product.name + " and " + " Customer : " + filter.customer.name
    }

    else {
      reportBy = 'Overview All Sales Report'
    }
    return reportBy
  }
          ...
          <div className={classes.root} ref={printRef}>
            <TableContainer component={Paper} className={classes.tableWrapper}>
              <div className={classes.header}>
                <Typography variant="h6" gutterBottom>
                  <a style={{ textDecoration: 'none', color: 'black', cursor: 'pointer' }}
                    href='#sales-list' target='_blank'>
                    {printItemHidden ?
                      <b> {getValues(filter)}</b>
                      :
                      <b>Sales Report
                        <Tooltip title="Go to sales page" placement="top"><OpenInNewIcon color='info' size='small' /></Tooltip></b>
                    }
                  </a>
                </Typography>
                {!printItemHidden && (
              ...
              <Table >
                  ...
                  {loading ? (
                    ...
                    rows && rows.length > 0 ? rows.map((row, idx) => (
                      <TableRow key={idx}>
                        ...
                        <TableCell style={{ textAlign: 'center' }}>₹{Number(row.qty) * Number(row.salesPrice)}</TableCell>
                      </TableRow>
                    )) : (
                      <TableRow >
                ...
                <TableFooter className={classes.tableHead}>
                  <TableRow>
                    ...
                    <TableCell style={{ fontSize: 16, textAlign: 'center' }}><strong>₹{rows?.reduce((acc, row) => acc + Number(row.qty) * Number(row.salesPrice), 0) || 0}</strong></TableCell>
                  </TableRow>
                </TableFooter>
              </Table>
              {!printItemHidden && <div className={classes.footer}>
                ...
                <Pagination
                  count={Math.ceil(totalRecords || 0 / rowsPerPage)}
                  page={page}

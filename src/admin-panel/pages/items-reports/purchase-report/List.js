...
const PurchaseReportList = () => {
  ...
  const fetchPurchaseData = () => {
    start();
    sendGetRequest(`${PURCHASE_REPORT}?startDate=${filter?.selectedRange.start}&endDate=${filter?.selectedRange.end}&product=${filter?.product ? filter?.product : ""}&supplier=${filter?.supplier?.id ? filter.supplier.id : ""}&per_page=${rowsPerPage}&page=${page}`, user.token)
      .then((_res) => {
        if (_res.status === 200) {
              ...
              <div className={classes.header}>
                <Typography variant="h6" gutterBottom>
                  <a style={{ textDecoration: 'none', color: 'black', cursor: 'pointer' }}
                    href='#purchase-list' target='_blank'>
                    {printItemHidden ?
                      <b> {getValues(filter)}</b>
                      :
                      <b>Purchase Report
                        <Tooltip title="Go to purchase page" placement="top"><OpenInNewIcon color='info' size='small' /></Tooltip></b>
                    }
                  </a>
                </Typography>
                {!printItemHidden && (
              ...
              <Table >
                  ...
                  {loading ? (
                    ...
                    rows && rows.length > 0 ? rows.map((row, idx) => (
                      <TableRow key={idx}>
                        ...
                        <TableCell style={{ textAlign: 'center' }}>₹{Number(row.qty) * Number(row.purchaseAmount)}</TableCell>
                      </TableRow>
                    )) : (
                      <TableRow >
                        <TableCell style={{ textAlign: 'center' }} colSpan={9}>No data found</TableCell>
                      </TableRow>
                    )
                  )}
                </TableBody>
                <TableFooter className={classes.tableHead}>
                  <TableRow>
                    ...
                    <TableCell style={{ fontSize: 16, textAlign: 'center' }}><strong>₹{rows?.reduce((acc, row) => acc + Number(row.qty) * Number(row.purchaseAmount), 0) || 0}</strong></TableCell>
                  </TableRow>
                </TableFooter>
              </Table>

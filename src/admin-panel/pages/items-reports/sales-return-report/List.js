...
const SalesReturnList = () => {
  ...
  const fetchSalesReturnData = () => {
    sendGetRequest(`${SALES_RETURN_OVERVIEW}?startDate=${filter?.selectedRange.start}&endDate=${filter?.selectedRange.end}&product=${filter?.product.name ? filter?.product.name : ""}&customer=${filter?.customer?.id ? filter.customer.id : ""}&per_page=${rowsPerPage}&page=${page}`, user.token)
      .then((_res) => {
        if (_res.status === 200) {
  ...
  const applyFilter = (selectedRange, customer, product) => {
    setFilter({ selectedRange, customer: customer, product });
    setPage(1);
  }
  const reset = () => {
    setFilter({ ...filter, selectedRange: { start: format(subMonths(new Date(), 3), 'yyyy-MM-dd'), end: format(new Date(), 'yyyy-MM-dd') }, product: '', customer: '' });
    setPage(1)
  }
  ...
  const getValues = (filter) => {
    let reportBy;
    if (filter && filter.product && !filter.customer) {
      reportBy = " Sales return report by Product : " + filter.product.name
    } else if (filter && filter.customer && !filter.product.name) {
      reportBy = " Sales return report by Customer : " + filter.customer.name
    } else if (filter && (filter.product.name && filter.customer)) {
      reportBy = " Sales return report by Product : " + filter.product.name + " and " + " Customer : " + filter.customer.name
    }
              ...
              <div className={classes.header}>
                <Typography variant="h6" gutterBottom>
                  <a style={{ textDecoration: 'none', color: 'black', cursor: 'pointer' }}
                    href='#sales-return' target='_blank'>
                    {printItemHidden ?
                      <b> {getValues(filter)}</b>
                      :
                      <b>Sales Return Report
                        <Tooltip title="Go to sales return page" placement="top"><OpenInNewIcon color='info' size='small' /></Tooltip></b>
                    }
                  </a>
                </Typography>
                {!printItemHidden && (
                  <div>
                    <Tooltip title="Refresh" placement="top">
                      <IconButton onClick={() => fetchSalesReturnData()}>
                        <Refresh />
                      </IconButton>
                    </Tooltip>
              ...
              <Table >
                <TableHead className={classes.tableHead}>
                  <TableRow>
                    ...
                    <TableCell style={{ textAlign: 'center' }}>Total Return Sales</TableCell>
                  </TableRow>
                </TableHead>
                <TableBody>
                  {loading ? (
                    ...
                    rows && rows.length > 0 ? rows.map((row, idx) => (
                      <TableRow key={idx}>
                        ...
                        <TableCell style={{ textAlign: 'center' }}>₹{row.salesReturnPrice}</TableCell>
                        <TableCell style={{ textAlign: 'center' }}>₹{Number(row.qty) * Number(row.salesReturnPrice)}</TableCell>
                      </TableRow>
                    )) : (
                      <TableRow>

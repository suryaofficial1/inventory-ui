...
import { PRODUCTION_PRODUCTS_LIST_BY_PRODUCT } from '../../config/api-urls';
...
const ProductionProductSpellSearch = ({ onSelect, clearSignal, status }) => {
    ...
    const getProducts = async (searchTerm) => {
        ...
        try {
            const res = await sendGetRequest(`${PRODUCTION_PRODUCTS_LIST_BY_PRODUCT}?product=${searchTerm}&status=${status}`, user.token);
            if (res.status === 200) {
...
export default ProductionProductSpellSearch;
